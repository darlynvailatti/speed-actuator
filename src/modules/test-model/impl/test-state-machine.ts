...
class TestStateMachineImpl {
  ...
  changeToNewState(newState: TestState) {
    if (!newState) throw new Error(`New state is mandatory`);
  ...
  private changeToReady() {
    ...
    // Clean any previous execution data
    this.test.testExecution = null;
  }
  ...
  private changeToStarted() {
    ...
    if (!isReady) throw new Error(`Test is not ready to start`);
  }
