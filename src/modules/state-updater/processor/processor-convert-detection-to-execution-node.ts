...
export default class ProcessorConvertDetectionToExecutionNode {
    ...
    private readonly logger = new Logger(ProcessorConvertDetectionToExecutionNode.name)
    ...
    private validateMandatoryFields(){
        ...
        if(!this.sensorDetectionMessage.sensor)
            throw new Error(`Can't convert an empty Sensor of SensorDetectionMessage`)

        if(!this.sensorDetectionMessage.sensor.code)
            throw new Error(`Can't convert an empty Code Sensor of SensorDetectionMessage`)

        if(!this.sensorDetectionMessage.timeStamp)
            throw new Error(`Can't convert an SensorDetectionMessage with empty TimeStamp`)

        if(!this.testTemplate)
            throw new Error(`Can't convert an SensorDetectionMessage without TestTemplate`)

        if(!this.testTemplate.graph || !this.testTemplate.graph.nodes)
            throw new Error(`Can't convert an SensorDetectionMessage with TestTemplate without or empty Graph`)
    }
}
